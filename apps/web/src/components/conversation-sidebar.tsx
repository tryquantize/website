...
export function ConversationSidebar({ onSelectConversation, isMinimized, onToggleMinimized, relatedQuestions = [], onQuestionClick, aiResponse, citations = [], currentQuery, conversationHistory = [] }: ConversationSidebarProps) {
  ...
  return (
    <>
      {/* Mobile Overlay */}
      {typeof window !== 'undefined' && window.innerWidth < 768 && !isMinimized && (
        <div 
      ...
      <div className={`fixed left-0 top-0 bottom-0 w-80 bg-white/5 backdrop-blur-2xl border-r border-white/15 flex flex-col ${typeof window !== 'undefined' && window.innerWidth < 768 ? 'z-50' : 'z-30'} ${typeof window !== 'undefined' && window.innerWidth < 768 ? 'top-0' : 'top-16'}`}>
      {/* Header */}
                ...
                <div className="space-y-0">
                  {item.relatedQuestions.map((question, qIndex) => (
                    <div key={qIndex}>
                      ...
                      {qIndex < item.relatedQuestions.length - 1 && (
                        <div className="border-b border-white/10" />
                      )}
                    </div>
                  ))}
