...
class ExaSearchService:
    ...
    def search_with_content(self, query: str, num_results: int = 1, include_domains: List[str] = None) -> Dict[str, Any]:
        """
        Search for content from specific URLs using Exa API
        """
        try:
            headers = {
                "Authorization": f"Bearer {self.api_key}",
            ...
            logger.error(f"Exa content search failed: {str(e)}")
            return {"success": False, "results": []}
